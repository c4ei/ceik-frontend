...
const WrongNetworkSelect = ({ switchNetwork, chainId }) => {
  ...
  const localChainId = useLocalNetworkChain() || ChainId.C4EI
  const [, setSessionChainId] = useSessionChainId()

  // const localChainName = chains.find((c) => c.id === localChainId)?.name ?? 'BSC'
  const localChainName = chains.find((c) => c.id === localChainId)?.name ?? 'C4EI'
...
export const NetworkSwitcher = () => {
  ...
  if (!chainId || chainId === ChainId.C4EI || chainId === ChainId.KLAY ) { return null }
  // if (!chainId || chainId === ChainId.BSC) { return null}
